...
# 8. 앱 하단 수정 로그 버튼
st.divider()
with st.expander("🛠️ 개발 및 수정 로그 확인하기"):
    st.write("""
    **[최종 업데이트: 2024.05.21]**
    - **UI/UX 개선**: 날짜 선택기를 연/월 드롭다운으로 변경하여 편의성 증대.
    - **자동 로직 추가**: 출생 연월 입력 시 현재 날짜 기준 개월 수 및 비교군(연령대) 자동 매칭 기능 구현.
    - **시각화 고도화**: Plotly를 활용한 인터랙티브 원형 그래프 병렬 배치.
    - **시나리오 테스트**: 연령별 정상/지체/자폐 발화 모델링 예시 데이터 구축.
    - **문구 최적화**: 학부모 심리를 고려한 부드러운 분석 메시지(UX Writing) 적용.
    - **안내 사항**: 수업 과제용 가상 수치 기준임을 명시하여 신뢰도 확보.
    """)
    st.info("본 서비스는 수업 과제용 가상 연령별 수치를 기준으로 분석한 결과입니다.")
