...
st.markdown("""
### 🎯 서비스 목적
이 앱은 인공지능(LLM)과 형태소 분석 기술을 활용하여 **학부모가 가정에서 아이의 언어 발달 상태를 객관적으로 모니터링**할 수 있도록 돕기 위해 제작되었습니다. 

1. **객관적 지표 제공**: 아이의 발화에서 명사, 동사, 형용사의 비중을 분석하여 또래 평균과 비교합니다.
2. **조기 발견 도움**: 언어 발달 지체나 특정 발화 패턴을 조기에 인지하고 전문가 상담의 필요성을 판단하는 기초 자료로 활용합니다.
3. **맞춤형 가이드**: 분석 결과에 따라 아이의 언어 발달을 자극할 수 있는 간단한 놀이 팁을 제공합니다.
""")
st.divider()
import datetime
